	...
	public partial class FrmRegistroReserva : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			try
			{
				...
				// Usamos la lógica de negocios para crear la reserva.
				LogicaReserva logicaReserva = new LogicaReserva();
				string resultado = logicaReserva.CrearReserva(reserva);
		...
		private void RefrescarVideojuegos()
		{
			try
			{
				...
				else
				{
					...
					// Preparamos la proyección para el DataGridView.
					var listaMostrar = listaVideojuegos.Select(v => new
					{
						Tienda = v.Tienda.Nombre,
						Videojuego = v.Videojuego.Nombre,
			...
			{
				MessageBox.Show("Error al refrescar la lista de videojuegos: " + ex.Message);
			}
		}
